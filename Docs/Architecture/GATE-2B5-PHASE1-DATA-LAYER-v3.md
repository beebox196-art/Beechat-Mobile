...
public func saveBridge(topicId: String, sessionKey: String) throws {
    try dbManager.write { db in
        ...
        try bridge.upsertPreservingCreatedAt(db)  // ← UPSERT — safe on duplicate
    }
}
```
...
public func connect() async {
    ...
    do {
        ...
        // 8. Re-subscribe to session updates (Kieran B8)
...
**Also update the navigation title** in `TopicListView`:
...
### 3.9 Seed Data — Create Topics, Not Sessions
...
9. Update `TopicListView`: change `Session` → `Topic`, update 3 property names
