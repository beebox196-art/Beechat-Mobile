...
**Status:** DRAFT — v3 revision (resolves v2 consolidated review blockers B1-B4 + warnings W1-W10)
...
| `BeeChatView.swift` | BeeChatUI | ✅ Session key resolution (Phase 1 fix) | Split into OnlineChatView + OfflineChatView, draft preservation |
...
| `EmptyTopicsView.swift` | BeeChatUI | Empty state for topic list (consistent "topics" terminology) |
| `OnlineChatView.swift` | BeeChatUI | Online chat with normal Exyte input (extracted from BeeChatView) |
| `OfflineChatView.swift` | BeeChatUI | Offline chat with disabled input + reconnect button (extracted from BeeChatView) |
...
- **Exyte generic type constraint:** `ChatView` with `inputViewBuilder` produces `ChatView<EmptyView, InputViewContent, DefaultMessageMenuAction>`. Without `inputViewBuilder` it produces `ChatView<EmptyView, EmptyView, DefaultMessageMenuAction>`. These are **different generic types** — they cannot coexist in the same `if/else` conditional. Must extract into separate sub-views.
- **`.presentationDetents` on `.popover`:** Detents only apply when the popover adapts to a sheet (iPhone/compact size class). On iPad where it stays a popover, detents are silently ignored. The `.frame` on the content controls popover size instead.
...
public func archiveTopic(id: String) throws -> Topic? {
    ...
    // Guard: don't re-archive an already-archived topic (avoids false undo toast)
...
Two new methods + one transaction helper:
...
/// On bridge failure (UNIQUE constraint), rolls back only the topic row
/// (NOT cascading — gateway messages must never be deleted by import rollback).
///
...
/// could insert between check and write. The catch block handles remaining cases.
///
...
public func importSelected(_ sessions: [Session]) throws -> Int {
    ...
    for session in sessions {
        ...
        // Create topic with the existing gateway session key
        let topic = Topic(
            id: UUID().uuidString,
        ...
        // Wrap save + bridge in a single write transaction for atomicity.
        // If saveBridge fails (UNIQUE constraint on openclawSessionKey),
        // the entire transaction rolls back — no orphaned topic left behind.
        // Crucially: we do NOT use deleteCascading() which would destroy
        // existing gateway messages linked to this session key.
        do {
            try persistenceStore.topicRepo.saveAndBridgeInTransaction(topic, sessionKey: session.id)
            count += 1
        } catch {
            // Transaction rolled back — topic was never persisted, bridge was never created.
            // No cleanup needed. Log for debugging.
...
struct EmptyTopicsView: View {
    ...
    let showArchiveToast: Bool      // when true, add bottom padding to avoid overlap
